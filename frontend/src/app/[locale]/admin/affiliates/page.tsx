...
export default function AdminAffiliatesPage() {
  ...
  useEffect(() => {
    ...
    getAdminAffiliates({ status: status || undefined, q: q || undefined, page })
      ...
      .catch((err) => {
        if (cancelled) return;
        setError(err instanceof Error ? err.message : t("affiliates.loadError"));
      })
      .finally(() => {
  ...
  const handleCreate = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : t("affiliates.createError"));
    } finally {
      setBusy(null);
    }
  };
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between gap-3">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">{t("affiliates.title")}</h1>
      ...
      {showForm && (
        ...
        >
          <div className="sm:col-span-2">
            <h2 className="text-sm font-semibold text-gray-900">{t("affiliates.accountLink")}</h2>
          </div>
          <div>
            <label className="mb-1 block text-sm font-medium text-gray-700">{t("affiliates.accountEmail")}</label>
            <input
              value={form.email}
          ...
          <div>
            <label className="mb-1 block text-sm font-medium text-gray-700">{t("affiliates.userIdOrEmail")}</label>
            <input
              value={form.user_id}
          ...
          <div className="sm:col-span-2">
            <h2 className="text-sm font-semibold text-gray-900">{t("affiliates.affiliateProfile")}</h2>
          </div>
          <div>
            <label className="mb-1 block text-sm font-medium text-gray-700">{t("affiliates.handle")}</label>
            <input
              value={form.handle}
          ...
          <div>
            <label className="mb-1 block text-sm font-medium text-gray-700">{t("affiliates.displayName")}</label>
            <input
          ...
          <div>
            <label className="mb-1 block text-sm font-medium text-gray-700">{t("affiliates.commissionBps")}</label>
            <input
          ...
          <div>
            <label className="mb-1 block text-sm font-medium text-gray-700">{t("affiliates.monthlyCap")}</label>
            <input
          ...
          <div className="sm:col-span-2">
            <h2 className="text-sm font-semibold text-gray-900">{t("affiliates.promoCode")}</h2>
          </div>
          <div>
          ...
          <div className="sm:col-span-2 flex justify-end">
            ...
            >
              ...
              {t("affiliates.createAndActivate")}
            </button>
          </div>
        </form>
      )}
      ...
      <div className="flex flex-wrap items-center gap-3">
        ...
        >
          <option value="">{t("affiliates.allStatuses")}</option>
          <option value="draft">{t("affiliates.statusDraft")}</option>
          <option value="active">{t("affiliates.statusActivePlural")}</option>
          <option value="suspended">{t("affiliates.statusSuspendedPlural")}</option>
        </select>
      </div>
        ...
        <div className="overflow-hidden rounded-2xl border border-gray-200 bg-white">
          <table className="w-full text-sm">
            <thead className="border-b border-gray-200 bg-gray-50 text-left text-xs uppercase tracking-wide text-gray-500">
              <tr>
                <th className="px-4 py-3 font-medium">{t("affiliates.affiliateHeader")}</th>
                <th className="px-4 py-3 font-medium">{t("affiliates.statusHeader")}</th>
                <th className="px-4 py-3 font-medium">{t("affiliates.commissionHeader")}</th>
                <th className="px-4 py-3 font-medium">{t("affiliates.balanceHeader")}</th>
                <th className="px-4 py-3 font-medium">{t("affiliates.codeHeader")}</th>
                <th className="px-4 py-3 font-medium">{t("affiliates.createdAtHeader")}</th>
                <th className="px-4 py-3 text-right font-medium">{t("affiliates.actionsHeader")}</th>
              </tr>
            </thead>
              ...
              {affiliates.map((a) => (
                <tr key={a.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-4 py-3">
                    ...
                    >
                      {statusLabelKeys[a.status] ? t(statusLabelKeys[a.status]) : a.status}
                    </span>
                  </td>
