...
const statusFilter = [
  { value: "", labelKey: "disputes.allStatuses" },
  { value: "open", labelKey: "disputes.statusOpenPlural" },
  { value: "moderating", labelKey: "disputes.statusModerating" },
  { value: "resolved", labelKey: "disputes.statusResolvedPlural" },
];

const statusBadge: Record<string, { cls: string; labelKey: string }> = {
  open: { cls: "bg-red-50 text-red-700", labelKey: "disputes.statusOpen" },
  moderating: { cls: "bg-amber-50 text-amber-700", labelKey: "disputes.statusModerating" },
  resolved: { cls: "bg-emerald-50 text-emerald-700", labelKey: "disputes.statusResolved" },
};
...
export default function AdminDisputesPage() {
  ...
  useEffect(() => {
    ...
    getAdminDisputes({ status: status || undefined, search: search || undefined, page })
      ...
      .catch((err) => {
        if (cancelled) return;
        setError(err instanceof Error ? err.message : t("disputes.loadError"));
      })
      .finally(() => {
  ...
  const openDetail = (id: number) => {
    ...
    getAdminDispute(id)
      ...
      .catch((err) => setActionError(err instanceof Error ? err.message : t("disputes.detailLoadError")))
  ...
  const resolve = () => {
    ...
    resolveAdminDispute(detail.id, {
      ...
      .catch((err) => setActionError(err instanceof Error ? err.message : t("disputes.resolveError")))
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-gray-600">{t("disputes.subtitle")}</p>
      </div>
        ...
        <div className="overflow-hidden rounded-2xl border border-gray-200 bg-white">
          <table className="w-full text-sm">
            <thead className="border-b border-gray-200 bg-gray-50 text-left text-xs uppercase tracking-wide text-gray-500">
                <tr>
                  <th className="px-4 py-3 font-medium">{t("disputes.disputeHeader")}</th>
                  <th className="px-4 py-3 font-medium">{t("disputes.buyerHeader")}</th>
                  <th className="px-4 py-3 font-medium">{t("disputes.shopHeader")}</th>
                  <th className="px-4 py-3 font-medium">{t("disputes.orderHeader")}</th>
                  <th className="px-4 py-3 font-medium">{t("disputes.statusHeader")}</th>
                  <th className="px-4 py-3 font-medium">{t("disputes.messagesHeader")}</th>
                  <th className="px-4 py-3 font-medium">{t("disputes.dateHeader")}</th>
                </tr>
            </thead>
            <tbody className="divide-y divide-gray-100">
              {disputes.map((dispute) => {
                ...
                return (
                  <tr key={dispute.id} onClick={() => void openDetail(dispute.id)} className="cursor-pointer hover:bg-emerald-50/40">
        ...
        <div className="rounded-2xl border border-gray-200 bg-white p-6">
            ...
            <div>
              <div className="flex items-center gap-2">
                ...
                <span className={`inline-flex rounded-full px-2.5 py-1 text-xs font-semibold ${
                  ...
                  {t((statusBadge[detail.status] ?? statusBadge.open).labelKey)}
                </span>
              </div>
              <p className="mt-1 text-sm text-gray-600">
                {detail.description ?? t("disputes.noDescription")}
              </p>
            </div>
            <button
              type="button"
          ...
          <div className="mt-4 grid gap-4 sm:grid-cols-2">
            <div className="rounded-xl bg-gray-50 p-4 text-sm">
              <p className="font-semibold text-gray-900">{t("disputes.buyer")}</p>
            ...
            <div className="rounded-xl bg-gray-50 p-4 text-sm">
              <p className="font-semibold text-gray-900">{t("disputes.shop")}</p>
              <p className="mt-1 text-gray-700">{detail.shop ?? "—"}</p>
              {detail.order && (
                <>
                  <p className="mt-1 text-xs text-gray-500">
                    {t("disputes.orderSummary", {
                      orderNumber: detail.order.order_number,
                      total: formatPrice(detail.order.total),
                    })}
                  </p>
                  <p className="text-xs text-gray-500">
                    {t("disputes.paymentSummary", {
                      paymentStatus: detail.order.payment_status,
                      status: detail.order.status_label,
                    })}
                  </p>
                </>
              )}
          ...
          <h3 className="mt-6 flex items-center gap-2 text-sm font-semibold text-gray-900">
            ...
            {t("disputes.messagesHeading", { count: detail.messages.length })}
          </h3>
          ...
          {detail.status !== "resolved" ? (
            <div className="mt-6 rounded-xl border border-emerald-100 bg-emerald-50/50 p-4">
              <h3 className="text-sm font-semibold text-gray-900">{t("disputes.resolution")}</h3>
              <div className="mt-3 grid gap-3">
                ...
                >
                  <option value="refund_full">
                    {t("disputes.fullRefund", { amount: formatPrice(detail.order?.total ?? 0) })}
                  </option>
                  <option value="refund_partial">{t("disputes.partialRefund")}</option>
                  <option value="no_refund">{t("disputes.noRefund")}</option>
                </select>
                {action === "refund_partial" && (
                  <input
                    ...
                    placeholder={t("disputes.amountPlaceholder")}
                    className="rounded-xl border border-gray-300 bg-white px-3 py-2 text-sm focus:border-emerald-500 focus:outline-none"
                  />
                )}
                <textarea
                  ...
                  rows={2}
                  placeholder={t("disputes.internalNotePlaceholder")}
                  className="rounded-xl border border-gray-300 bg-white px-3 py-2 text-sm focus:border-emerald-500 focus:outline-none"
                />
                <div className="flex items-center gap-3">
                  ...
                  {detail.order && (
                    <span className="text-xs text-gray-500">
                      {t("disputes.maxAvailableAmount", {
                        amount: formatPrice(detail.order.total),
                      })}
                    </span>
                  )}
                </div>
            ...
            detail.resolution && (
              <div className="mt-6 rounded-xl bg-emerald-50 p-4 text-sm text-emerald-800">
                <p className="font-semibold">{t("disputes.resolved")}</p>
