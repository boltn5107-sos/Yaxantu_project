...
export default function AdminOrderDetailPage() {
  ...
  useEffect(() => {
    ...
    getAdminOrder(params.orderNumber)
      ...
      .catch((err) => {
        if (cancelled) return;
        setError(err instanceof Error ? err.message : t("orders.notFoundError"));
      })
      .finally(() => {
  ...
  const saveStatus = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : t("orders.updateError"));
    } finally {
      setBusy(null);
    }
  };
  ...
  const handleCancel = async () => {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : t("orders.cancelError"));
    } finally {
      setBusy(null);
    }
  };
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between gap-3">
        <div>
          <Link href="/admin/orders" className="inline-flex items-center gap-1 text-sm font-medium text-emerald-700 hover:text-emerald-800">
          ...
          <p className="text-sm text-gray-600">
            {t("orders.orderDate", {
              date: order.placed_at ? new Date(order.placed_at).toLocaleString("fr-FR") : "—",
            })}{" "}
            {t("orders.statusLabel")} <span className="font-semibold capitalize">{order.status_label}</span>
          </p>
        </div>
      ...
      <form onSubmit={saveStatus} className="flex flex-col gap-3 rounded-2xl border border-gray-200 bg-white p-5 sm:flex-row sm:items-end">
        <div className="flex-1">
          <label className="mb-1 block text-sm font-medium text-gray-700">{t("orders.changeStatus")}</label>
          <select
        ...
        <button
          ...
          className="inline-flex items-center gap-2 rounded-xl bg-emerald-600 px-6 py-2 text-sm font-semibold text-white hover:bg-emerald-700 disabled:opacity-60"
        >
      ...
      <div className="grid gap-6 lg:grid-cols-3">
        <div className="rounded-2xl border border-gray-200 bg-white p-5 lg:col-span-2">
          <h2 className="mb-4 text-base font-semibold text-gray-900">
            {t("orders.itemsHeading", { count: order.items_count })}
          </h2>
          ...
          <div className="mt-4 space-y-2 border-t border-gray-200 pt-4 text-sm">
            ...
            {order.discount > 0 && (
              <div className="flex justify-between font-medium text-emerald-700">
                <span>{t("orders.discount", { code: order.promo_code ? `(${order.promo_code})` : "" })}</span>
                <span>-{formatPrice(order.discount)}</span>
              </div>
            )}
            <div className="flex justify-between text-gray-600">
              <span>{t("orders.shipping")}</span>
              <span>{order.shipping === 0 ? t("orders.freeShipping") : formatPrice(order.shipping)}</span>
            </div>
        ...
        <div className="space-y-6">
          <div className="rounded-2xl border border-gray-200 bg-white p-5">
            <h2 className="mb-3 flex items-center gap-2 text-base font-semibold text-gray-900">
              <UserIcon className="h-4 w-4 text-gray-500" /> {t("orders.customer")}
            </h2>
            <p className="text-sm font-medium text-gray-900">{order.customer ?? "—"}</p>
            <p className="text-sm text-gray-500">{order.customer_email ?? "—"}</p>
            <p className="text-sm text-gray-500">{order.customer_phone ?? "—"}</p>
            <p className="mt-2 text-sm text-gray-600">
              {t("orders.shopLabel")} <span className="font-medium">{order.shop ?? "—"}</span>
            </p>
          </div>
          ...
          {order.address && (
            <div className="rounded-2xl border border-gray-200 bg-white p-5">
              <h2 className="mb-3 flex items-center gap-2 text-base font-semibold text-gray-900">
                <MapPin className="h-4 w-4 text-gray-500" /> {t("orders.deliveryAddress")}
              </h2>
          ...
          {order.delivery && (
            <div className="rounded-2xl border border-gray-200 bg-white p-5">
              <h2 className="mb-3 text-base font-semibold text-gray-900">{t("orders.delivery")}</h2>
              <p className="text-sm capitalize text-gray-700">
                {t("orders.statusValue", { status: order.delivery.status.replace("_", " ") })}
              </p>
              <p className="text-sm text-gray-600">
                {t("orders.carrier", {
                  provider: order.delivery.provider ?? t("orders.internalCarrier"),
                })}
              </p>
              {order.delivery.tracking_number && (
                <p className="text-sm text-gray-600">
                  {t("orders.tracking", { trackingNumber: order.delivery.tracking_number })}
                </p>
              )}
            </div>
          )}
          ...
          {order.payment && (
            <div className="rounded-2xl border border-gray-200 bg-white p-5">
              <h2 className="mb-3 text-base font-semibold text-gray-900">{t("orders.payment")}</h2>
              <p className="text-sm capitalize text-gray-700">
                {t("orders.statusValue", { status: order.payment.status.replace("_", " ") })}
              </p>
              <p className="text-sm text-gray-600">
                {t("orders.method", { method: order.payment.method ?? "—" })}
              </p>
            </div>
          )}
        </div>
