  ...
  return (
    <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-10">
      <div className="overflow-hidden rounded-3xl border border-gray-200 bg-white">
        <div className="h-36 bg-gradient-to-r from-emerald-600 via-emerald-700 to-emerald-900" />
        <div className="relative px-6 pb-8 sm:px-10">
          <div className="-mt-14 flex flex-col sm:flex-row sm:items-end gap-4">
            {shop.logo ? (
              ...
              <div className="flex h-24 w-24 items-center justify-center rounded-2xl border-4 border-white bg-emerald-600 shadow-lg">
            ...
            <div className="flex-1 pt-4 sm:pt-0">
              ...
              <div className="mt-1 flex flex-wrap items-center gap-x-4 gap-y-1 text-sm text-gray-600">
                ...
                <span className="flex items-center gap-1">
                  ...
                  {t("shop.salesCount", { count: shop.sales_count })}
                </span>
                {shop.location && (
                  <span className="flex items-center gap-1">
              ...
              {user && !isOwner && (
                <button
                  ...
                  onClick={() => {
                    ...
                    if (!user) {
                      router.push("/auth/login?next=/seller/"+slug);
                      return;
                    }
                    setStartingChat(true);
                    startConversation(shop.id)
                      .then((conversation) =>
                        router.push(`/messages/${conversation.id}`),
                      )
                      .catch(() => setStartingChat(false));
                  }}
                  className="inline-flex items-center gap-2 rounded-xl bg-emerald-600 px-4 py-2.5 text-sm font-semibold text-white hover:bg-emerald-700 transition-colors disabled:opacity-60"
                >
                  <MessageCircle className="h-4 w-4" />
                  {startingChat ? t("shop.chat.opening") : t("shop.chat.action")}
                </button>
              )}
              <button
                type="button"
          ...
          <div className="mt-5 flex flex-wrap gap-2">
            {shop.category && (
              ...
              >
                {categoryName(ct, shop.category.name, shop.category.slug)}
              </Link>
            )}
      ...
      <div className="mt-10">
        <div className="mb-6 flex items-center justify-between">
          ...
          <span className="text-sm text-gray-600">
            {t("shop.products.count", { count: data.meta.total_products })}
          </span>
        </div>
        {products.length === 0 ? (
