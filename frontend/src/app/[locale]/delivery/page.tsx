...
export default function DeliveryPage() {
  ...
  useEffect(() => {
    // Décalé hors du corps synchrone de l'effet (appel de load() qui
    // synchronise des états), pour éviter un rendu en cascade.
  ...
  const toggleAvailability = async () => {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : t("availabilityError"));
    } finally {
      setBusy(null);
    }
  };
  ...
  const pickup = async (deliveryId: number) => {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : t("pickupError"));
    } finally {
      setBusy(null);
    }
  };
  ...
  const deliver = async (deliveryId: number, received: boolean) => {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : t("confirmationError"));
    } finally {
      setBusy(null);
    }
  };
  ...
  if (progress && !progress.is_onboarded) {
    return (
      <StandaloneNotice
        title={t("finishRegistrationTitle")}
        text={t("finishRegistrationText")}
        href="/delivery/onboarding"
        cta={t("resumeRegistrationCta")}
      />
    );
  }
    ...
    return (
      <div className="mx-auto max-w-md px-4 py-16 text-center">
        ...
        <p className="mt-2 text-sm text-gray-600">
          {t("pendingMessage", { hours: progress.review_hours })}
        </p>
      </div>
    );
  }
  ...
  return (
    <div className="mx-auto max-w-4xl px-4 sm:px-6 py-10">
      ...
      >
        <div>
          <p className="text-sm font-medium text-gray-900">
            {progress?.available ? t("availableTitle") : t("unavailableTitle")}
          </p>
      ...
      <h2 className="mb-3 text-lg font-semibold text-gray-900">{t("jobsTitle")}</h2>
      ...
      {jobs.length === 0 ? (
        ...
        <div className="space-y-4">
          {jobs.map((job) => {
            const statusKey = jobStatusKeys[job.status as keyof typeof jobStatusKeys];
            return (
            <div key={job.delivery_id} className="rounded-2xl border border-gray-200 bg-white p-5">
                ...
                <span className="text-xs text-gray-500">
                  {t("assignedAt", {
                    date: job.assigned_at
                      ? new Date(job.assigned_at).toLocaleTimeString("fr-FR")
                      : "—",
                  })}
                </span>
              </div>
              ...
              <div className="mt-4 grid gap-3 sm:grid-cols-2">
                <div className="rounded-xl bg-gray-50 p-3">
                  ...
                  <a href={`tel:${job.seller_phone}`} className="mt-1 inline-flex items-center gap-1 text-sm font-medium text-emerald-700">
                ...
                <div className="rounded-xl bg-gray-50 p-3">
                  ...
                  <div className="mt-1 flex items-center gap-2">
                    <a href={`tel:${job.buyer_phone}`} className="inline-flex items-center gap-1 text-sm font-medium text-emerald-700">
                      <Phone className="h-3.5 w-3.5" />
                      {job.buyer_phone}
                    </a>
                  </div>
