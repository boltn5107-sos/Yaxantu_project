...
const segments: { id: SellerSegment; label: string }[] = [
          ...
          <button
            ...
            onClick={() =>
              void act(order.order_number, () =>
                sellerRefuseOrder(order.order_number, "Refusée par le vendeur."),
              )
            }
    ...
    <SellerOnboardingGate>
      ...
      <div className="mb-6 flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-sm text-gray-600">Acceptez, expédiez et livrez en un geste.</p>
        </div>
      </div>

      <div className="mb-6 flex gap-1 rounded-xl border border-gray-200 bg-white p-1">
        {segments.map((seg) => {
          const count = seg.id === segment ? state?.meta?.total : undefined;
      ...
      {loading ? (
        ...
        <div className="rounded-2xl border border-dashed border-gray-300 bg-white py-16 text-center text-gray-500">
          Aucune commande dans cette liste. Les nouvelles commandes arrivent ici.
        </div>
      ) : (
      ...
      <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
        <div className="flex-1">
          ...
          <p className="mt-1 text-xs text-gray-500">
            {order.buyer.name ?? "Client"} · {order.buyer.phone ?? "—"} ·{" "}
...
function StatusPill({ status, label, color }: { status: string; label: string; color: string }) {
  ...
  if (label.startsWith("En attente") || label.includes("attente")) {
    return (
