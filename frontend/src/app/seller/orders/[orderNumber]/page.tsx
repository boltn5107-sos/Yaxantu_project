  ...
  if (error || !order) {
    return (
      <div className="mx-auto max-w-3xl px-4 py-16 text-center">
        ...
        <Link href="/seller/orders" className="mt-4 inline-block text-sm font-medium text-blue-700 hover:underline">
    ...
    <div className="mx-auto max-w-4xl px-4 sm:px-6 py-10">
      ...
      {canShip && (
        <button
          ...
          className="mb-6 w-full inline-flex items-center justify-center gap-2 rounded-xl bg-blue-600 px-4 py-3 text-sm font-semibold text-white hover:bg-blue-700 disabled:opacity-60"
        >
          {busy === "ship" ? <Loader2 className="h-4 w-4 animate-spin" /> : <Truck className="h-4 w-4" />}
          Expédier (préparer le colis pour le livreur)
        </button>
      )}
      ...
      {canDeliver && (
        <button
          ...
          disabled={busy !== null}
          onClick={() =>
            void act("deliver", () => sellerMarkDelivered(orderNumber, "Remise confirmée par le vendeur."))
          }
        ...
        >
          ...
          Confirmer que le client a reçu (l&apos;argent est débloqué)
        </button>
      )}
      ...
      <div className="grid gap-5 md:grid-cols-2">
        <div className="rounded-2xl border border-gray-200 bg-white p-6">
          <h2 className="mb-4 flex items-center gap-2 text-base font-semibold text-gray-900">
            ...
            Montants — tout en clair
          </h2>
          <dl className="space-y-3 text-sm">
            <Row label="Vente" value={formatPrice(order.detail.sale_price)} />
            <Row label="Livraison" value={formatPrice(order.detail.shipping)} />
            <div className="border-t border-dashed border-gray-200 pt-3">
              <Row
                label="Commission Yaxantu"
                value={`- ${formatPrice(order.detail.platform_commission)}`}
                muted
              />
              <Row label="Frais de paiement" value={`- ${formatPrice(order.detail.payment_fee)}`} muted />
            </div>
            <div className="border-t border-gray-200 pt-3">
              <div className="flex items-center justify-between">
                <dt className="font-semibold text-gray-900">Net reçu</dt>
                <dd className="font-bold text-emerald-700">{formatPrice(order.detail.net_received)}</dd>
              </div>
              <p className="mt-1 text-xs text-gray-500">
                Une fois le client livré, ce montant passe dans votre solde disponible.
              </p>
            </div>
          </dl>
        </div>
          ...
          <div className="rounded-2xl border border-gray-200 bg-white p-6">
            ...
            {order.address ? (
              <div className="text-sm text-gray-700">
                ...
                {order.address.phone && <p className="mt-1 font-medium text-blue-700">{order.address.phone}</p>}
              </div>
            ) : (
            ...
            {order.delivery?.courier && (
              <div className="mt-4 rounded-xl bg-blue-50 p-4">
                ...
                <p className="mt-1 text-sm font-semibold text-gray-900">
                  {order.delivery.courier.name ?? "Livreur Yaxantu"}
                </p>
                {order.delivery.courier.phone && (
                  <p className="text-sm text-blue-700">{order.delivery.courier.phone}</p>
                )}
                {order.delivery.courier.transport && (
                  <p className="text-xs text-blue-600">
                    Transport : {order.delivery.courier.transport}
                  </p>
                )}
          ...
          {order.dispute && (
            <div className="rounded-2xl border border-red-200 bg-red-50 p-6">
              ...
              <p className="text-sm text-red-700">
                {order.dispute.messages_count} message{order.dispute.messages_count > 1 ? "s" : ""} · une
                personne de confiance s&apos;occupe de votre litige.
              </p>
            </div>
          )}
        </div>
            ...
            <div key={item.id} className="flex items-center justify-between py-3">
              <div className="flex items-center gap-3">
                {item.image ? (
                  ...
                  <img src={item.image} alt="" className="h-12 w-12 rounded-xl border border-gray-200 object-cover" />
                ) : (
