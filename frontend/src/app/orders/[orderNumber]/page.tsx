...
const PROGRESS_STEPS = [
  { label: "Commande confirmée", icon: CheckCircle2 },
  { label: "Paiement reçu", icon: CreditCard },
        ...
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-10">
          <div className="rounded-2xl border border-gray-200 bg-gray-50 p-16 text-center">
            <p className="text-lg font-medium text-gray-900">
              {errorStatus === 404
                ? "Cette commande est introuvable ou ne vous appartient pas."
    ...
    <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-10">
      <div className="mb-8">
        <div className="flex items-center gap-2 text-sm text-gray-600 mb-2">
          <Link href="/orders" className="hover:text-blue-700 transition-colors">
            Commandes
          </Link>
        ...
        {created && (
          <div className="mt-4 rounded-xl border border-emerald-200 bg-emerald-50 px-4 py-3 text-sm text-emerald-800">
            Votre commande a bien été enregistrée. Le paiement sera confirmé
            après validation (COD : à la livraison).
          </div>
        )}
        {actionError && (
                ...
                order.items.map((item) => (
                  ...
                  >
                    <div className="h-16 w-16 flex-shrink-0 overflow-hidden rounded-lg bg-gray-100">
                      ...
                      <img
                        src={item.image || fallbackImage(120, 120, 2)}
                        alt={item.name}
                    ...
                    <div className="flex-1 min-w-0">
                      {item.slug ? (
                        <Link href={`/product/${item.slug}`} className="hover:text-blue-700 transition-colors">
                      ...
                      <p className="text-xs text-gray-600">
                        ...
                        {item.seller ? ` — Vendeur : ${item.seller}` : ""}
                      </p>
                    </div>
                ...
                <div className="flex items-start gap-3 text-gray-700">
                  ...
                  <div>
                    ...
                    {order.payment.transaction_id && (
                      <p className="text-xs text-gray-500">
                        Réf : {order.payment.transaction_id}
                      </p>
                    )}
                  </div>
                </div>
